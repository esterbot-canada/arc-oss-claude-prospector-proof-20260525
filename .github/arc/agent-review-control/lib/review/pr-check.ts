...
export function createArcPrCheck(input: ArcPrCheckInput): ArcPrCheckResult {
  ...
  const receipts: string[] = [
    ...
    `Diff source: ${input.diffSource ? `${input.diffSource.description} (${input.diffSource.trust})` : 'unknown'}`,
    'Command receipts must be trusted_ci to count as machine-verified evidence. Agent-reported receipts are reviewer hints only.',
  ];
  ...
  for (const command of commands.satisfiedCommands) {
    if (trustedReceiptCommands.has(command)) {
      receipts.push(`Trusted CI required command receipt reported exit 0: ${command}`);
    } else {
      focusQuestions.push(`Was '${command}' verified by CI/provider evidence? Impact: agent-reported command receipts can be forged.`);
      receipts.push(`Agent-reported required command receipt reported exit 0: ${command}`);
    }
  }
  ...
  const summaryReason =
    status === 'Blocked'
      ...
      : status === 'Needs Review'
        ...
        : 'Changed files stayed inside approved scope and trusted CI required command receipts reported exit 0.';
