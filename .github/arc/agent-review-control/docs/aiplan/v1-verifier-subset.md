...
- **Pass** exits 0.
- **Needs Review** exits 0 in the GitHub action so reviewers see the Trust Brief without a fake red CI failure.
- **Blocked** exits 1.

For Needs Review, the action also emits a GitHub warning annotation so the check is not visually silent/green-only.
